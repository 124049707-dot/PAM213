...
export default function InsertUsuarioScreen() {
  ...
  // --- LÓGICA DE ELIMINAR CON ALERTA ---
  const eliminarUsuario = (id) => {
    if (guardando) return;

    // CASO 1: WEB (Usamos window.confirm)
    if (Platform.OS === 'web') {
    ...
    // CASO 2: MÓVIL (Usamos Alert.alert nativo)
    else {
      Alert.alert(
        "Eliminar Usuario", // Título
        "¿Estás seguro de que deseas eliminarlo? Esta acción no se puede deshacer.", // Mensaje
        [
  ...
  // Función auxiliar que realmente borra (solo se llama si confirmas)
  const ejecutarBorrado = async (id) => {
    try {
      setGuardando(true);
      await controller.eliminarUsuario(id);
      // No necesitamos Alert de éxito porque se ve visualmente que desaparece
    } catch (e) {
  ...
  const guardarEdicion = async () => {
    if (!nuevoNombreEditar.trim()) return;
    
    try {
      if (usuarioAEditar) {
        await controller.actualizarUsuario(usuarioAEditar.id, nuevoNombreEditar);
  ...
  const renderUsuario = ({ item, index }) => (
    <View style={styles.userItem}>
      ...
      <View style={styles.actionButtons}>
        ...
        {/* Al presionar aquí se dispara la alerta de confirmación */}
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>
        {Platform.OS === 'web' ? 'WEB (LocalStorage)' : 'MOVIL (SQLite)'}
      </Text>
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity onPress={guardarEdicion} style={styles.modalBtnSave}>
...
const styles = StyleSheet.create({
  ...
  insertSection: { backgroundColor: '#fff', padding: 20, marginHorizontal: 15, marginBottom: 15, borderRadius: 12, shadowOpacity: 0.1, elevation: 3 },
  selectSection: { flex: 1, backgroundColor: '#fff', marginHorizontal: 15, marginBottom: 15, borderRadius: 12, padding: 15, shadowOpacity: 0.1, elevation: 3 },
  
  selectHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
  ...
  actionButtons: { justifyContent: 'center', alignItems: 'flex-end', marginLeft: 10 },
  textEdit: { color: '#007AFF', fontWeight: 'bold', marginBottom: 10, fontSize: 14 },
  textDelete: { color: '#FF3B30', fontWeight: 'bold', fontSize: 14 },

  centeredView: { flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: 'rgba(0,0,0,0.4)' },
  modalView: { width: '80%', backgroundColor: "white", borderRadius: 12, padding: 25, shadowColor: "#000", elevation: 5 },
  modalTitle: { fontSize: 18, fontWeight: 'bold', marginBottom: 15, textAlign: 'center' },
  modalButtons: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 10 },
  modalBtnCancel: { padding: 10 },
  modalBtnSave: { backgroundColor: '#007AFF', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 5 },
